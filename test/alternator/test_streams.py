...
def test_streams_spurious_modify_when_update_expr_on_missing_item(test_table_ss_new_and_old_images, dynamodb, dynamodbstreams):
    ...
    def do_updates(table, p, c):
        ...
        table.update_item(Key={'p': p, 'c': 'c0'}, UpdateExpression='REMOVE g')
        v = table.get_item(Key={'p': p, 'c': 'c0'}).get('Item', None)
        assert v is None

        # then we try to remove existing column from existing item
        table.update_item(Key={'p': p, 'c': 'c0'}, UpdateExpression="SET e = :e, g = :g", ExpressionAttributeValues={':e': 166, ':g': 166})
        v = table.get_item(Key={'p': p, 'c': 'c0'})['Item']
        assert v == {'p': p, 'c': 'c0', 'e': 166, 'g': 166}
        events.append(['INSERT',{'c':'c0','p':p},null,{'c':'c0','e':166,'g':166,'p':p}])

        table.update_item(Key={'p': p, 'c': 'c0'}, UpdateExpression='REMOVE g')
        v = table.get_item(Key={'p': p, 'c': 'c0'})['Item']
        assert v == {'p': p, 'c': 'c0', 'e': 166}
        events.append(['MODIFY',{'c':'c0','p':p},{'c':'c0','e':166,'g':166,'p':p}, {'c':'c0','e':166,'p':p}])
...
# the same as test_streams_spurious_modify_when_update_expr_on_missing_item but for a table without clustering key
def test_streams_spurious_modify_when_update_expr_on_missing_item_on_no_clustering_key_table(test_table_s_no_ck_new_and_old_images, dynamodb, dynamodbstreams):
    null = None
    def do_updates(table, p, c):
        events = [
            ['INSERT',{'p':p},null,{'e':166,'g':166,'p':p}],
            ['MODIFY',{'p':p},{'e':166,'g':166,'p':p}, {'e':166,'p':p}],
        ]
        # first we try to remove column from non existing item
...
def test_table_ss_new_and_old_images_write_isolation_always(dynamodb, dynamodbstreams):
    with create_table_ss(dynamodb, dynamodbstreams, 'NEW_AND_OLD_IMAGES', additional_tags=[
        {'Key': 'system:write_isolation', 'Value': 'always_use_lwt'}]) as stream:
        yield stream
