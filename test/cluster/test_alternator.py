...
async def test_alternator_listen_socket_backlog(manager: ManagerClient):
    ...
    config = {**alternator_config, 'alternator_listen_socket_backlog': initial_backlog}
    servers = await manager.servers_add(1, config=config)
    server = servers[0]
    port = alternator_config['alternator_port']

    # Verify the initial backlog value was applied
    ...
    # Live-update the backlog and verify it takes effect
    ...
    async def backlog_updated():
        if get_listen_backlog(server.ip_addr, port) == updated_backlog:
            return True
        return None
