...
static mutation make_export_metadata_mutation(schema_ptr schema, api::timestamp_type ts,
    ...
    add_timestamp_cell(m, *schema, ts, "accepted_at", export_time);
    return m;
}
...
static future<> persist_export_metadata(service::storage_proxy& proxy, service_permit permit,
        ...
        int64_t export_time, std::string_view export_type) {
    ...
    mutations.push_back(make_export_metadata_mutation(exports_schema, ts, export_arn, table_arn, client_token, request_json, export_time));
...
static rjson::value make_export_description(std::string_view export_arn, const query::result_set_row& row) {
    ...
    rjson::add(export_desc, "ExportTime", rjson::value(export_time && export_time->IsNumber() ? static_cast<int64_t>(export_time->GetDouble()) : accepted_at));
...
static future<std::optional<rjson::value>> existing_export_for_client_token(service::storage_proxy& proxy, service::client_state& client_state,
    ...
    if (*request_json != rjson::print(rjson::copy(request))) {
