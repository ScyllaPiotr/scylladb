...
// Debug printer for cdc::stream_id. We print both signed and unsigned value as we use both.
template <>
...
public:
    ...
    stream_arn(schema_ptr s, schema_ptr base_schema) {
        ...
        auto table_index = _arn.find(":000000000000:table/") + std::string_view{":000000000000:table/"}.size();
...
// parent-child relationship is based on token ranges, which are
// unbroken parts of token space. Token is a signed integer and
// token space is whole range of values a token can take.
// For vnodes the space "wraps around" (so lowest integer is
// one value from biggest one). To be able to find parent (or child)
// we need to sort stream ids by token so we could use `lower_bound`
// since we don't know much about the sorting order of the incoming data
// we will explicitly sort them using `compare_by_token` function as
...
// It seems KCL is depending on it, so we need to adhere.
...
void stream_id_range::update_starting_position(const cdc::stream_id &update_to) {
    _skip_to = &update_to;
}
...
// the function returns `stream_id_range` that will allow iteration over children stream shards for the shard `parent`
// a child stream shard is defined as a stream shard that touches token range that was previously covered by `parent` shard
// stream shard contains a token, that represents end of the token range for that stream shard (inclusive)
// begginning of the token range is defined by previous stream shard's token + 1
...
//
// to find children we will first find parent stream shard in parent_streams by its token
// then we will find previous stream shard in parent stream - that will determine range
// then based on the range we will find children stream shards in current_streams
...
stream_id_range find_children_range_from_parent_token(
    ...
    auto child_shard_end_it = std::lower_bound(current_streams.begin(), current_streams.end(), parent_shard_end_it->token(), [](const cdc::stream_id& id, const dht::token& t) {
            return id.token() < t;
    });
    ...
    if (uses_tablets) {
        ...
        //   - find lower or equal token in C to parent_shard_begin_it, set it to child_shard_begin_it
        //      - if child_shard_begin_it->token() == parent_shard_begin_it->token() then increment child_shard_begin_it
        //      child_shard_begin_it now points at first child with token > parent_shard_begin_it->token()
        // - else previous one to parent_shard_end_it does not exist
        //   - set child_shard_begin_it = C.begin()
        // - find lower or equal token in C to parent_shard_end_it, set it to child_shard_end_it, increment once
        //      child_shard_end_it now points at first child with token > parent_shard_end_it
        // - range [child_shard_begin_it, child_shard_end_it) represents children
        
        if (child_shard_end_it == current_streams.end()) {
            // last token is not present, we bail out early with an internal error
            on_internal_error(slogger, fmt::format("last token from token space not present in tokens, for parent shard id {}, got parent shards [{}] and children shards [{}]", 
        ...
        if (parent_shard_end_it == parent_streams.begin()) {
            ...
            child_shard_begin_it = std::lower_bound(current_streams.begin(), current_streams.end(), parent_shard_begin_it->token(), [](const cdc::stream_id& id, const dht::token& t) {
                return id.token() < t;
            });
            // sanity check - child_shard_begin_it must not be end() - last token must be present
            if (child_shard_begin_it == current_streams.end()) {
                on_internal_error(slogger, fmt::format("last token from token space not present in tokens, for shard id {}, parent shard id {}, got parent shards [{}] and children shards [{}]", 
            ...
            if (child_shard_begin_it->token() == parent_shard_begin_it->token()) {
                ++child_shard_begin_it;
            }
        }
        ...
        if (parent_shard_begin_it == parent_streams.begin()) {
            ...
            // P=|     -30 -20      |
            // C=| -40          -10 |
            // searching for parent stream shard at -30 will get us here - end of the parent is -30, beginning is -20
...
future<executor::request_return_type> executor::describe_stream(client_state& client_state, service_permit permit, rjson::value request) {
    ...
    rjson::add(stream_desc, "StreamArn", alternator::stream_arn(schema, bs));
    ...
    for (; limit > 0 && i != e; prev = i, ++i) {
        ...
        if (shard_start) {
            shard_range->update_starting_position(shard_start->id);
        }
