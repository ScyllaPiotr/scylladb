...
// Kinesis Client Library does check for ARN format, hence we hardcode beginning as
// `arn:aws:dynamodb:us-east-1:000000000000` - those values were chosen only to make KCL accept our ARN
static rjson::value generate_arn_for_table(const schema& schema) {
    return rjson::from_string(fmt::format("arn:aws:dynamodb:us-east-1:000000000000:table/{}@{}", schema.ks_name(), schema.cf_name()));
}
...
arn_parts parse_arn(std::string_view arn, std::string_view arn_field_name, std::string_view type_name, std::string_view expected_postfix) {
    ...
    auto keyspace_name = table_name.substr(0, separator_pos);
